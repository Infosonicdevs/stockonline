...
import { getPurchaseRegister } from "../../services/reports/purchaseRegister";
...
function PurchaseRegister() {
  ...
  const fetchReport = async () => {
    ...
    setLoading(true);
    try {
      const response = await getPurchaseRegister(fromDate, toDate);
      if (response.data) {
  ...
  return (
    <div className="container-fluid py-3">
      <div className="card shadow-sm border-0">
        ...
        >
          <h5 className="mb-0">
            <i className="bi bi-file-earmark-spreadsheet me-2"></i>Purchase Register
          </h5>
        </div>
              ...
              <tbody className="text-center">
                {reportData.length > 0 ? (
                  reportData.map((item, index) => (
                    <tr key={index}>
                      <td>{index + 1}</td>
                      <td>{item.Invoice_Date ? new Date(item.Invoice_Date).toLocaleDateString("en-GB") : "-"}</td>
